...
a=input("type in a twitter handle")
...
umich_tweets=get_user_tweets(a)
...
connect=sqlite3.connect('project3_tweets.db')
cur=connect.cursor()
...
new_table+= 'Tweets (tweet_id TEXT PRIMARY KEY, text TEXT, user_id TEXT, time_posted TIMESTAMP, retweets INTEGER)'
cur.execute(new_table)

new_table= 'CREATE TABLE IF NOT EXISTS '
new_table+='Users (user_id TEXT PRIMARY KEY, screen_name TEXT, num_favs INTEGER, descriptions TEXT)'
cur.execute(new_table)
...
for items in umich_tweets:
	...
	text = items["text"]
	user_id = items["user"]["id_str"]
	retweets = items["retweet_count"]
	time_posted = items["created_at"]
	
	tw_tup=(tweet_id,text,user_id,time_posted,retweets)
	list_1.append(tw_tup)
...
statement_='INSERT OR IGNORE INTO Users VALUES (?,?,?,?)'

for items in umich_tweets: 
	users = items["entities"]["user_mentions"]
	for q in users:
		p = api.get_user(q["screen_name"])
		cur.execute(statement_,(p["id_str"],p["screen_name"],p["favourites_count"],p["description"]))
...
sql2='SELECT screen_name FROM Users'
cur.execute(sql2)
...
sql3='SELECT * FROM Tweets WHERE retweets>10'
cur.execute(sql3)
...
sql4='SELECT descriptions FROM USERS WHERE num_favs>25'
cur.execute(sql4)
...
sql5=('SELECT Users.screen_name,Tweets.text FROM Users INNER JOIN Tweets On Users.user_id=Tweets.user_id WHERE retweets>10')
cur.execute(sql5)
